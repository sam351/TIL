...
## dplyr - arrange() : 오름차순, arrange(desc()) : 내림차순
exam %>% arrange(math)  # math 오름차순 정렬
# 내림차순으로 정렬하기
exam %>% arrange(desc(math))  # math 내림차순 정렬
# 정렬 기준 변수 여러개 지정
exam %>% arrange(class, math)  # class 및 math 오름차순 정렬
exam %>% arrange(class, desc(math))  # class 오름차순 및 math 내림차순 정렬

exam %>% arrange(desc(math)) %>% head(1)  # 전체 중 수학 점수 가장 높은 행 리턴
...
## dplyr - summarise() : 요약하기, group_by(변수명), n()
exam %>% summarise(mean_math = mean(math))  # math 평균 산출
mean(exam$math)  # ∴ 전체에 대해 요약할 때는 summarise 불필요
# 집단별로 요약하기
exam %>%
